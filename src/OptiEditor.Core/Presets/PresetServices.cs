...
public sealed class UserPresetStore(string? appData = null, IDiagnosticLogger? logger = null) : IUserPresetStore
{
    ...
    public async Task<IReadOnlyList<PresetDefinition>> LoadAsync(CancellationToken token = default)
    {
        ...
        try { await using var stream = File.OpenRead(_path); return (await JsonSerializer.DeserializeAsync<List<PresetDefinition>>(stream, cancellationToken: token) ?? []).Where(x => x.Source == PresetSource.User).ToArray(); }
    ...
    public async Task SaveAsync(IEnumerable<PresetDefinition> presets, CancellationToken token = default)
    {
        ...
        var gate = Locks.GetOrAdd(_path, _ => new(1, 1));
...
public sealed class PresetValidationService(OptiSchemaResolver resolver)
{
    public string? Validate(PresetDefinition preset, IEnumerable<PresetDefinition>? existing = null)
    {
        ...
        var schema = resolver.Resolve(preset.Family);
        foreach (var entry in preset.Entries) if (schema.FindById(entry.SettingId) is null || !SettingValidator.Validate(schema.FindById(entry.SettingId)!, entry.RawValue).IsValid) return $"Invalid preset entry: {entry.SettingId}.";
        return null;
    }
}
...
public sealed class PresetCaptureService(OptiSchemaResolver resolver)
{
    ...
    {
        var schema = resolver.Resolve(family);
        var entries = values.Where(x => x.IsModified).Where(x => schema.FindById(x.SettingId) is { } definition && SettingValidator.Validate(definition, x.RawValue).IsValid).Select(x => new PresetEntry(x.SettingId, x.RawValue)).ToArray();
