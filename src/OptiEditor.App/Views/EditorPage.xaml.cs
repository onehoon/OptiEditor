...
public sealed partial class EditorPage : Page
{
    ...
    private void RenderSettings()
    {
        ...
        foreach (var item in ViewModel.Settings)
        {
            ...
            Grid.SetColumn(input, 1); grid.Children.Add(input); var actions = new StackPanel { Spacing = 4 }; actions.Children.Add(new Button { Content = "Revert", Tag = item }); if (item.HasBlockingValidationError && !string.IsNullOrWhiteSpace(item.ValidationError)) actions.Children.Add(new TextBlock { Text = item.ValidationError, Foreground = new Microsoft.UI.Xaml.Media.SolidColorBrush(Microsoft.UI.Colors.IndianRed), TextWrapping = TextWrapping.Wrap, MaxWidth = 180 }); Grid.SetColumn(actions, 2); grid.Children.Add(actions);
