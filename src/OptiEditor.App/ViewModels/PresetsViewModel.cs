...
public partial class PresetsViewModel : ObservableObject
{
 ...
 {
  var users = (await AppServices.Presets.LoadAsync()).ToList(); var error = new PresetValidationService(AppServices.Schemas).Validate(preset, users); if (error is not null) return error;
